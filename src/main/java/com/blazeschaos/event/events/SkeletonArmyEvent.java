...
public final class SkeletonArmyEvent extends ChaosEvent {
    public SkeletonArmyEvent() { super("skeleton-army", "Skeleton Army"); }
    @Override public void start(@NotNull GameInstance game) {
        int amount = Math.max(1, (int) Math.round(settingInt("mobs-per-player", 8) * mobScale(game)));
        ThreadLocalRandom r = ThreadLocalRandom.current();
        for (Player player : game.getAlivePlayers()) {
            for (int i = 0; i < amount; i++) {
                game.trackEntity(player.getWorld().spawn(
                        player.getLocation().add(r.nextInt(-6, 7), 0, r.nextInt(-6, 7)), Skeleton.class));
            }
        }
    }
